...
function MessageArea() {
  ...
  return (
    <div className="w-full h-[100vh] bg-black relative">
      ...
      <div
        className="w-full h-[80%] pt-[100px] px-[40px] flex flex-col gap-[50px] 
      overflow-auto bg-black"
      >
        {messages && messages.map((mess,index)=>(
            mess.sender == userData._id ?<SenderMessage message={mess} key={index}/>:<ReceiverMessage message={mess}/>
        ))}
      </div>

      <div
        className="w-full h-[80px] fixed bottom-0 flex justify-center items-center
      bg-black z-[100]"
      >
        <form
          className="w-[90%] max-w-[800px] h-[80%] rounded-full bg-[#131616] flex items-center
        gap-[10px] px-[20px] relative" onSubmit={handleSendMessage}
        >
